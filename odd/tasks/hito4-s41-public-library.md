...
- **`.env.local` points at the HOSTED project** (`https://kspnacfcietqikbufcka.supabase.co`), not the local stack. If the app is opened in a browser today it talks to the hosted DB, where 0010/0011 + seed are NOT applied. The slice was verified against the local stack (docker). Applying to the hosted project is a delivery decision, not part of this slice's smoke.