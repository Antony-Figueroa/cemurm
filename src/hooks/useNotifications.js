...
//
// Off2 delta-summary synthesis + consume land in 4.6 — the `summary` state
// slot and `consumeSummary` are part of the 2.3 return contract so the page
// surface is stable; the summary is synthesized only on reconcile refetches.

import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { useAuth } from './useAuth.jsx'
import * as notifications from '../lib/notifications.js'
import { offlineGet, offlineSet } from '../lib/offlineCache.js'
...
// Off2 last-seen baseline (4.6): the hook's OWN additive kv key — the
// `notifications:*` keys belong to the data layer (notifications.js), so this
// one lives under a distinct name; no store/schema/DB_VERSION change.
const LAST_SEEN_KEY = (userId) => `notifications-last-seen:${userId}`
...
export function useNotifications() {
  ...
  // 4.6 Off2 recipient side: synthesize the offline summary from the fetch
  // delta (unread − lastSeenUnread), grouped by category. Runs only on
  // reconcile refetches (online / cemurm:sync-done / focus) — mount and the
  // realtime echo refetch plain so no stale "pending" banner is announced.
  const synthesizeSummary = useCallback(async (fetchedRows, fetchedUnread) => {
    if (!user) return
    const key = LAST_SEEN_KEY(user.id)
    const lastSeen = await offlineGet(key)
    const lastSeenUnread = lastSeen?.data ?? null
    if (lastSeenUnread === null) {
      // First reconcile: baseline the unread count, announce nothing.
      await offlineSet(key, fetchedUnread)
      setSummary(null)
      return
    }
    const delta = fetchedUnread - lastSeenUnread
    if (delta <= 0) {
      setSummary(null)
      return
    }
    // byCategory over the newest `delta` unread rows (feed is newest-first).
    const newestUnread = fetchedRows.filter((row) => !row.read).slice(0, delta)
    setSummary({
      count: delta,
      byCategory: notifications.groupByCategory(newestUnread).map((g) => ({
        category: g.category,
        count: g.unread,
      })),
    })
  ...
  const refresh = useCallback(async (opts = {}) => {
    ...
    try {
      ...
      if (reconcile) await synthesizeSummary(fetchedRows, fetchedUnread)
    } finally {
      setLoading(false)
    }
  }, [user, synthesizeSummary])

  // Read on mount, when connectivity returns, when an offline drain commits,
  // and when the tab regains focus (design focus-refetch fallback). The
  // online / sync-done / focus refetches are reconcile refetches (Off2).
  useEffect(() => {
    ...
    const refetch = () => refresh({ reconcile: true })
  ...
  // 4.6 Off2: consume clears the banner AND persists lastSeenUnread = current
  // unread, so the next reconcile diff starts from the consumed point.
  const consumeSummary = useCallback(async () => {
    if (!user) return
    setSummary(null)
    await offlineSet(LAST_SEEN_KEY(user.id), unreadRef.current)
  }, [user])
